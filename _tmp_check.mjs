    ...
    function connectSocket() {
      ...
      ws.addEventListener('open', () => {
        ...
        if (pendingJoinName) {
          ws.send(JSON.stringify({ type: 'join', name: pendingJoinName }));
          pendingJoinName = '';
        }
      });
      ...
      ws.addEventListener('close', () => {
        ...
        showToast('Połączenie z serwerem zostało przerwane. Łączenie ponownie...');
    ...
    makeRoadPlane(118, 8, 0, 0);
    makeRoadPlane(8, 118, 0, 0);
    makeRoadPlane(34, 5, -14, 10, 0);
    makeRoadPlane(34, 5, 14, -10, 0);
    ...
    function buildObstacle(def, wallMap, roofMap, glassMap, metalMap, crateMap, woodMap) {
      ...
      if (def.kind === 'cave') {
        const outer = new THREE.Mesh(
          new THREE.BoxGeometry(def.w, def.h, def.d),
          new THREE.MeshStandardMaterial({ map: textures.rock, roughness: 1, metalness: 0.02, color: 0x64748b })
        );
        outer.position.set(def.x, def.h / 2, def.z);
        outer.castShadow = outer.receiveShadow = true;
        obstacleGroup.add(outer);

        const arch = new THREE.Mesh(
          new THREE.CylinderGeometry(Math.max(def.w, def.d) * 0.42, Math.max(def.w, def.d) * 0.55, def.d * 0.9, 10, 1, false, 0.2, Math.PI - 0.4),
          new THREE.MeshStandardMaterial({ color: 0x0f172a, roughness: 1, metalness: 0.0 })
        );
        arch.rotation.z = Math.PI / 2;
        arch.position.set(def.x, def.h * 0.58, def.z);
        arch.castShadow = arch.receiveShadow = true;
        obstacleGroup.add(arch);
        ...
        for (const [ox, oz, scale] of stones) {
          const stone = new THREE.Mesh(
            new THREE.SphereGeometry(Math.max(def.w, def.d) * 0.12 * scale, 8, 8),
            new THREE.MeshStandardMaterial({ color: 0x334155, roughness: 1 })
          );
          stone.position.set(def.x + ox * def.w, def.h * 0.25, def.z + oz * def.d);
      ...
      if (def.kind === 'mega') {
        ...
        const leftTower = new THREE.Mesh(
          new THREE.CylinderGeometry(def.w * 0.14, def.w * 0.18, def.h * 1.15, 8),
          towerMat
        );
        leftTower.position.set(def.x - def.w * 0.32, def.h * 0.575, def.z - def.d * 0.24);
        leftTower.castShadow = leftTower.receiveShadow = true;
        obstacleGroup.add(leftTower);

        const rightTower = leftTower.clone();
        rightTower.position.x = def.x + def.w * 0.32;
        obstacleGroup.add(rightTower);

        const neon = new THREE.PointLight(0x22d3ee, 0.7, 14, 2);
        neon.position.set(def.x, def.h + 1.8, def.z);
        scene.add(neon);
        return;
      }
    ...
    function updateSceneFromState(state) {
      ...
      if (me) {
        ...
        const roleKey = `${me.side || 'none'}|${me.roleName || 'none'}`;
        if (roleKey !== lastRoleKey) {
          lastRoleKey = roleKey;
          roleBannerUntil = now + 5000;
        }

        if (me.roleName) {
          bannerText = `${roleSide}: ${me.roleName}`;
          bannerSub = me.roleDesc ? `Umiejętność: ${me.roleDesc}` : '';
        } else {
          bannerText = roleSide;
          bannerSub = state.message || '';
        }
      } else {
        ...
        bannerText = state.running ? 'Czekamy na przypisanie roli' : 'Czekamy na graczy';
        bannerSub = state.message || '';
      }

      const blind = !!(me && me.blindUntil && now < me.blindUntil);
      const safe = !!(me && me.safeUntil && now < me.safeUntil);
      const bannerVisible = !blind && state.running && now < roleBannerUntil;
      ui.centerTitle.textContent = bannerText;
      ui.centerSub.textContent = bannerSub;
      ui.centerMessage.classList.toggle('show', bannerVisible);
